...
__all__ = ["playsound", "stopsound"]

minecraft_sounds = Literal[
...
@command
def playsound(sound: minecraft_sounds, source: sound_sources, targets: Selector, pos: Coords = None, volume: float = None, pitch: float = None, min_volume: float = None):
    out = f"playsound {sound} {source} {targets}"
    if pos:
        out += f" {pos}"
    if volume:
        out += f" {float(volume)}"
    if pitch:
        out += f" {float(pitch)}"
    if min_volume:
        out += f" {float(min_volume)}"
    return out


@command
def stopsound(targets: Selector, source: sound_sources = None, sound: minecraft_sounds = None):
    out = f"stopsound {targets}"
    if source:
        out += f" {source}"
    if sound:
        out += f" {sound}"
